...
import type { Client, ConceptVariant, Project, ShareLink } from "@/lib/types";
...
export default async function SharePage({ params, searchParams }: SharePageProps) {
  ...
  const link = shareLink as ShareLink | null;
  ...
  const projectData = project as (Project & { client: Client | null }) | null;
  if (!projectData) {
    notFound();
  }

  if (link.view_type === "export") {
    ...
    const [brief, outputs, feedback, references, keyVisualGroups, concepts, scripts, conceptAssets] =
      await Promise.all([
        getLatestBrief(supabase, link.project_id),
        getOutputs(supabase, link.project_id),
        getFeedback(supabase, link.project_id),
        getReferences(supabase, link.project_id),
        getPrimaryKeyVisualsForProject(supabase, link.project_id),
        listConcepts(supabase, link.project_id),
        listScripts(supabase, link.project_id),
        listConceptAssetsByProject(supabase, link.project_id),
      ]);
    ...
    const storyboard = primaryScript ? await getStoryboard(supabase, primaryScript.id) : null;
    ...
    return (
      <div className="space-y-6">
        <div className="space-y-2">
          ...
          <p className="text-sm text-muted-foreground">{projectData.client?.name ?? "Client"}</p>
          {link.label ? (
            <p className="text-xs text-muted-foreground">Label: {link.label}</p>
          ) : null}
        </div>
        ...
        <div className="space-y-6">
          <Card>
            ...
            <CardContent className="space-y-2 text-sm">
              <p>
                <strong>Client:</strong> {projectData.client?.name ?? ""}
              </p>
              <p>
  ...
  const [creativeSpec, concepts, scripts, outputs, feedback, references, conceptAssets] =
    await Promise.all([
      getCreativeSpec(supabase, link.project_id),
      listConcepts(supabase, link.project_id),
      listScripts(supabase, link.project_id),
      getOutputs(supabase, link.project_id),
      getFeedback(supabase, link.project_id),
  ...
  const variantsByConceptEntries = await Promise.all(
    concepts.map(async (concept) => {
      const variants = await listVariants(supabase, concept.id);
  ...
  return (
    <div className="space-y-6">
      <div className="space-y-2">
        ...
        <h2 className="text-2xl font-semibold">{projectData.name}</h2>
        <p className="text-sm text-muted-foreground">{projectData.client?.name ?? "Client"}</p>
        {link.label ? <p className="text-xs text-muted-foreground">Label: {link.label}</p> : null}
      </div>
